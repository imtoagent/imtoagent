  ...
  private async _processMessageInternal(
    ...
    while (attempt <= maxRetries) {
      ...
      try {
        ...
        // Phase 3: 拦截 Goal 管理指令（优先于正常回复发送）
        const goalResult = this.config.goalManager?.processManagementCommand(output.text, ctx.chatId);
        if (goalResult) {
          console.log(`[Runtime] 🎯 Goal management command intercepted: ${goalResult.slice(0, 80)}`);
          await ctx.reply(goalResult);
          this.config.sessionManager.persist(botName, session);
          return { restart: false };
        }
