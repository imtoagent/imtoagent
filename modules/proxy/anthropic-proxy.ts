...
function notifyUserError(status: number) {
  ...
  bot.notifyUser(msg).catch(e => logger.error('proxy/anthropic-proxy', `notifyUser failed: ${e.message}`));
}
...
export function loadProviders(): { providers: Map<string, ProviderConfig>; defaultModel: string } {
  ...
  try {
    ...
    logger.info('proxy/anthropic-proxy', `Loaded ${providers.size} provider(s) from config.json: ${[...providers.keys()].join(', ')}`);
  } catch (e: unknown) {
...
export function saveActiveModel(modelSpec: string): void {
  ...
  try {
    ...
    logger.info('proxy/anthropic-proxy', `[saveActiveModel] config.json.activeModel → ${modelSpec}`);
  } catch (e: unknown) {
    logger.error('proxy/anthropic-proxy', `[saveActiveModel] Failed: ${(e as Error).message}`);
  }
}
...
export function calculateCost(modelSpec: string, inputTokens: number, outputTokens: number): number {
  try {
    ...
    // 未知供应商 — 输出警告日志
    logger.warn('proxy/anthropic-proxy', `[calculateCost] ⚠️ Unknown provider "${provider}", using default pricing ($0.55/M input, $2.19/M output)`);
  } catch {}
...
function handleRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
  ...
  req.on('end', async () => {
    ...
    logger.info('proxy/anthropic-proxy', `Model resolved: ${originalModel} → ${resolvedSpec}`);
    ...
    if (breaker && !breaker.canRequest()) {
      logger.debug('proxy/anthropic-proxy', `⚡ Circuit breaker OPEN for ${targetProviderName}, trying failover...`);
      logEvent({
